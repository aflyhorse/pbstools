...
class jobinfo:
    ...
    def __eq__(self, other):
        ...
        if (
            ...
            or self.get_state() != other.get_state()
            or self.get_update_time() != other.get_update_time()
        ):
            return False
        ignore_rsrcs = [
            "qtime",
        ...
        for key in list(set(self.get_resource_keys()) | set(other.get_resource_keys())):
            if key not in ignore_rsrcs:
                if not self.has_resource(key) or not other.has_resource(key):
                    return False
                elif key in [
                    "resources_used.cput",
                    "resources_used.walltime",
                ] and time_to_sec(self.get_resource(key)) != time_to_sec(
                    other.get_resource(key)
                ):
                    return False
    ...
    def __repr__(self):
        ...
        output += "\tlast_update_time = %s (%d)\n" % (
            str(self.get_update_time()),
            self.get_update_time_ts(),
        )
        output += "\tjobname = %s\n" % self.name()
        output += "\tqueue = %s\n" % self.queue()
        output += "\tuser = %s\n" % self.user()
        ...
        if self.walltime_used_sec() > 0:
            output += "\twalltime_used = %s (%d)\n" % (
                sec_to_time(self.walltime_used_sec()),
                self.walltime_used_sec(),
            )
            output += "\twalltime_limit = %s (%d)\n" % (
                sec_to_time(self.walltime_limit_sec()),
                self.walltime_limit_sec(),
            )
        if self.cput_used_sec() > 0:
            output += "\tcput_used = %s (%d)\n" % (
                sec_to_time(self.cput_used_sec()),
                self.cput_used_sec(),
            )
            output += "\tcput_limit = %s (%d)\n" % (
                sec_to_time(self.cput_limit_sec()),
                self.cput_limit_sec(),
            )
    ...
    def add_to_resource(self, key, value):
        ...
        if key in supported_time_resources and not self.has_resource(key):
            self._resources[key] = value
    ...
    def submithost(self):
        owner = self.owner()
        if owner is None or "@" not in owner or len(owner.split("@")) > 2:
            return None
        else:
    ...
    def ctime(self):
        if self.has_resource("ctime"):
            ...
            raise RuntimeError(
                "Job " + self._jobid + " has no ctime set (aborted before started?)"
            )
    ...
    def qtime(self):
        if self.has_resource("qtime"):
            ...
            raise RuntimeError(
                "Job " + self._jobid + " has no qtime set (aborted before started?)"
            )
    ...
    def etime(self):
        if self.has_resource("etime"):
            ...
            raise RuntimeError(
                "Job " + self._jobid + " has no etime set (aborted before started?)"
            )
    ...
    def start(self):
        if self.has_resource("start"):
            ...
            raise RuntimeError(
                "Job "
                + self._jobid
                + " has no start time set (aborted before started?)"
            )
    ...
    def num_processors(self):
        ...
        if (
            self.has_resource("Resource_List.mppssp")
            or self.has_resource("resources_used.mppssp")
            or self.has_resource("Resource_List.mppe")
            or self.has_resource("resources_used.mppe")
        ):
...
class jobinfoTestCase(unittest.TestCase):
    ...
    def test_tasks(self):
        ...
        if "16057.pitzer-batch.ten.osc.edu" not in jobs:
            ...
            self.assertEqual(
                j2.tasks(),
                "1:lprocs=1:usecores:memory=4GB+1:lprocs=1:usecores:memory=4GB",
            )
        try:
            os.unlink(tmpfile)
        except Exception:
    ...
    def test_write_last_accounting_record(self):
        ...
        self.assertEqual(
            fd1.getvalue(),
            "02/13/2009 18:31:30;E;123456.fakehost.lan;Resource_List.cput=2:00:00 Resource_List.mem=1GB Resource_List.nodes=2:ppn=4 Resource_List.vmem=1GB Resource_List.walltime=1:00:00 ctime=1234567890 end=1234567890 etime=1234567890 exec_host=node01/1+node02/2 exit_status=0 group=bar jobname=job owner=foo@login1.fakehost.lan qtime=1234567890 queue=batch resources_used.cput=00:00:02 resources_used.mem=1024kb resources_used.vmem=2048kb resources_used.walltime=00:00:01 start=1234567890 start_count=1 system=None user=foo\n",
        )
        fd1.close()
...
def raw_data_from_file(filename):
    ...
    for line in acct_data:
        ...
        try:
            ...
            print(
                "ERROR:  Invalid number of fields (requires 4).  Unable to parse entry: %s"
                % (str(line.split(";", 3)))
            )
            continue
        ...
        for resource in resources.split(" "):
            ...
            if match:
                ...
                logger.warn(
                    'filename=%s, jobid=%s:  Malformed resource "%s"'
...
def jobinfo_from_epilogue(
    jobid, reqlist="", usedlist="", queue=None, account=None, exit_status=0, system=None
):
    """
...
class pbsacctTestCase(unittest.TestCase):
    ...
    def test_jobinfo_from_epilogue(self):
        j1 = jobinfo_from_epilogue(
            "123456.testfakehost.lan",
            "cput=2:00:00,mem=1GB,nodes=2:ppn=4,vmem=1GB,walltime=1:00:00,neednodes=2:ppn=4",
            "cput=00:00:02,mem=1024kb,vmem=2048kb,walltime=00:00:01",
            "batch",
        )
        self.assertEqual(j1.numeric_jobid(), 123456)
        self.assertEqual(j1.account(), None)
...
class pbsacctDB:
        ...
        if self.getType() == "mysql":
            try:
                ...
                try:
                    ...
                    raise ImportError(
                        "MySQL Python library not found. Install python3-mysqlclient or PyMySQL"
                    )
            self._dbhandle = MySQLdb.connect(
                host=self._dbhost,
                db=self._dbname,
                user=self._dbuser,
            ...
            self._dbhandle = psycopg2.connect(
                host=self._dbhost,
                db=self._dbname,
                user=self._dbuser,
                passwd=self._dbpasswd,
    ...
    def job_exists(self, jobid, append_to_jobid=None):
        ...
        if len(results) == 0:
            ...
            raise RuntimeError(
                "More than one result for jobid %s (should not be possible)" % jobid
            )
    ...
    def _timestamp_to_date(self, ts):
        if self.getType() in ["mysql"]:
            ...
            raise RuntimeError(
                'Unable to determine ts->date conversion for database type "%s"'
                % self.getType()
            )
    ...
    def _job_set_fields(self, job, system=None, oldjob=None, append_to_jobid=None):
        ...
        if system is not None and (oldjob is None or job.system() != oldjob.system()):
            fields_to_set["system"] = "'%s'" % system
        if job.user() is not None and (oldjob is None or job.user() != oldjob.user()):
        ...
        if job.name() is not None and (oldjob is None or job.name() != oldjob.name()):
        ...
        if job.gres() is not None and (oldjob is None or job.gres() != oldjob.gres()):
        ...
        if job.qos() is not None and (oldjob is None or job.qos() != oldjob.qos()):
        ...
        if job.end_ts() > 0 and (oldjob is None or job.end_ts() != oldjob.end_ts()):
        ...
        if job.walltime_limit_sec() > 0 and (
            ...
            fields_to_set["walltime_req"] = "'%s'" % sec_to_time(
                job.walltime_limit_sec()
            )
        ...
        if (
            job.has_resource("Resource_List.mppe")
            or job.has_resource("resources_used.mppe")
        ) and (
            oldjob is None
            or (
                job.get_resource("Resource_List.mppe")
                != oldjob.get_resource("Resource_List.mppe")
                or job.get_resource("resources_used.mppe")
                != oldjob.get_resource("resources_used.mppe")
            )
        ):
            fields_to_set["mppe"] = "'%d'" % max(
                int(job.get_resource("Resource_List.mppe")),
                int(job.get_resource("resources_used.mppe")),
            )
        if (
            job.has_resource("Resource_List.mppssp")
            or job.has_resource("resources_used.mppssp")
        ) and (
            oldjob is None
            or (
                job.get_resource("Resource_List.mppssp")
                != oldjob.get_resource("Resource_List.mppssp")
                or job.get_resource("resources_used.mppssp")
                != oldjob.get_resource("resources_used.mppssp")
            )
        ):
            fields_to_set["mppssp"] = "'%d'" % max(
                int(job.get_resource("Resource_List.mppssp")),
                int(job.get_resource("resources_used.mppssp")),
            )
        if job.has_resource("exec_host") and (
            oldjob is None
            or job.get_resource("exec_host") != oldjob.get_resource("exec_host")
        ):
            fields_to_set["hostlist"] = "'%s'" % job.get_resource("exec_host")
        if job.exit_status() is not None and (
            oldjob is None
            or job.get_resource("Exit_status") != oldjob.get_resource("Exit_status")
        ):
        ...
        if self.getType() == "mysql":
            for timefield in ["cput", "cput_req", "walltime", "walltime_req"]:
                if (
                    timefield in fields_to_set
                    and time_to_sec(fields_to_set[timefield].strip("'")) >= 839 * 3600
                ):
    ...
    def insert_job(
        self, job, system=None, check_existance=True, noop=False, append_to_jobid=None
    ):
        if not isinstance(job, jobinfo):
            raise TypeError('"job" object is of wrong type:  %s' % str(job))
        if check_existance and self.job_exists(
            job.jobid(), append_to_jobid=append_to_jobid
        ):
            raise RuntimeError(
                "Job %s already exists in database, cannot insert" % job.jobid()
            )
    ...
    def update_job(
        ...
        if not isinstance(job, jobinfo):
            raise TypeError('"job" object is of wrong type:  %s' % str(job))
        if check_existance and not self.job_exists(
            job.jobid(), append_to_jobid=append_to_jobid
        ):
            raise RuntimeError(
                "Job %s does not exist in database, cannot update" % job.jobid()
            )
        myjobid = job.jobid()
        if append_to_jobid is not None:
            myjobid = job.jobid() + append_to_jobid
        ...
        if job != oldjob:
            delta = self._job_set_fields(
                job, system, oldjob, append_to_jobid=append_to_jobid
            )
            if delta is not None:
                deltalist = []
                for key in sorted(delta.keys()):
                    deltalist.append("%s=%s" % (key, delta[key]))
                sql = "UPDATE %s SET %s WHERE jobid='%s'" % (
                    self.getJobsTable(),
                    ", ".join(deltalist),
                    myjobid,
                )
    ...
    def insert_or_update_job(self, job, system=None, noop=False, append_to_jobid=None):
        ...
        if self.job_exists(job.jobid(), append_to_jobid=append_to_jobid):
            self.update_job(
                ...
                noop=noop,
                append_to_jobid=append_to_jobid,
            )
        else:
            self.insert_job(
                job,
            ...
            if noop:
                ...
                if len(results) == 0:
                    ...
                    for i in range(len(result)):
                        if (
                            columns[i] in ["account", "jobname", "queue", "system"]
                            and result[i] is not None
                        ):
                            resources[columns[i]] = str(result[i])
                        elif columns[i] == "username" and result[i] is not None:
                            resources["user"] = str(result[i])
                        elif columns[i] == "groupname" and result[i] is not None:
                            resources["group"] = str(result[i])
                        elif columns[i] == "submithost" and result[i] is not None:
                            if "user" in resources:
                                resources["owner"] = (
                                    resources["user"] + "@" + str(result[i])
                                )
                        elif columns[i] == "submit_ts" and result[i] > 0:
                            resources["ctime"] = str(result[i])
                            resources["qtime"] = str(result[i])
                        elif columns[i] == "eligible_ts" and result[i] > 0:
                            resources["etime"] = str(result[i])
                        elif columns[i] == "start_ts" and result[i] > 0:
                            resources["start"] = str(result[i])
                        elif columns[i] == "end_ts" and result[i] > 0:
                            resources["end"] = str(result[i])
                        elif columns[i] == "start_count" and result[i] > 0:
                            resources["start_count"] = str(result[i])
                        elif columns[i] == "hostlist" and result[i] is not None:
                            resources["exec_host"] = str(result[i])
                        elif columns[i] == "exit_status" and result[i] is not None:
                            ...
                            if isinstance(result[i], datetime.timedelta):
                                if result[i].days > 0 and result[i].seconds > 0:
                                    resources["Resource_List.cput"] = sec_to_time(
                                        24 * 3600 * result[i].days + result[i].seconds
                                    )
                                else:
                            ...
                            if "Resource_List.walltime" not in resources or (
                                "Resource_List.walltime" in resources
                                and result[i] is not None
                                and int(result[i])
                                > time_to_sec(resources["Resource_List.walltime"])
                            ):
                                resources["Resource_List.walltime"] = sec_to_time(
                                    result[i]
                                )
                            ...
                            if "resources_used.cput" not in resources:
                                if isinstance(result[i], datetime.timedelta):
                                    resources["resources_used.cput"] = sec_to_time(
                                        24 * 3600 * result[i].days + result[i].seconds
                                    )
                                elif isinstance(result[i], int):
                                    resources["resources_used.cput"] = sec_to_time(
                            ...
                            if "resources_used.cput" not in resources or (
                                "resources_used.cput" in resources
                                and result[i] is not None
                                and int(result[i])
                                > time_to_sec(resources["resources_used.cput"])
                            ):
                                resources["resources_used.cput"] = sec_to_time(
                                    result[i]
                                )
                            ...
                            if "resources_used.walltime" not in resources:
                                if isinstance(result[i], datetime.timedelta):
                                    ...
                                    resources["resources_used.walltime"] = str(
                                        result[i]
                                    )
                        elif columns[i] == "walltime_sec":
                            if "resources_used.walltime" not in resources or (
                                "resources_used.walltime" in resources
                                and result[i] is not None
                                and int(result[i])
                                > time_to_sec(resources["resources_used.walltime"])
                            ):
                                resources["resources_used.walltime"] = sec_to_time(
                                    result[i]
                                )
                    ...
                    job = jobinfo(
                        myjobid,
                        datetime.datetime.fromtimestamp(float(updatetime)).strftime(
                            "%m/%d/%Y %H:%M:%S"
                        ),
                        state,
                        resources,
                    ...
                    raise RuntimeError(
                        "More than one result for jobid %s (should not be possible)"
                        % jobid
                    )
